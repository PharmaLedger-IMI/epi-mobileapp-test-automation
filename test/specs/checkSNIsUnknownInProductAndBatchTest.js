...
describe('091_Edit product to check SN is unknown and pass unknown SN in matrix', () => {
    ...
    it('Mobile App - should display LeafLet Details to check SN is unknown and pass unknown SN in matrix', async () => {
        allureReporter.addFeature('Leaflet displayed for Edit product to check SN is unknown and pass unknown SN in matrix');
        ...
        await timeoutWait.setTimeoutWait(3);
        await checkSnIsUnknownInProductAndBatch.getLeafletTypesAndLevel();
        await timeoutWait.setTimeoutWait(3);
